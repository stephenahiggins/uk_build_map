...
const UpdateProjectPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      if (project.statusRationale) setValue('statusRationale', project.statusRationale);
      if (project.expectedCompletion) {
        setValue('expectedCompletion', project.expectedCompletion.split('T')[0]);
      }
      setLoading(false);
    };
    fetchProject();
    ...
    <div className="max-w-xl mx-auto p-8 bg-white rounded shadow mt-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <textarea {...register('statusRationale')} className="input input-bordered w-full h-24" />
        </div>
        <div>
          <label className="block font-medium">Expected Completion</label>
          <input type="date" {...register('expectedCompletion')} className="input input-bordered w-full" />
        </div>
