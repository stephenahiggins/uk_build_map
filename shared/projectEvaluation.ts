...
function normaliseStatus(value: string | undefined | null): "Red" | "Amber" | "Green" {
...
function buildEvidenceNarrative(items: EvidenceForEvaluation[]): string {
  ...
  return items
    .map((item, index) => {
      ...
      const source = item.source ? `Source: ${item.source}` : "Source: (unknown)";
      const url = item.sourceUrl ? `URL: ${item.sourceUrl}` : "URL: (unknown)";
      const date = item.evidenceDate ? `Date: ${item.evidenceDate}` : "Date: (unknown)";
...
export async function evaluateProjectWithGemini(
  ...
  const { apiKey, model, enableGoogleSearch = true, mockResponse } = options || {};
  ...
  const ragRationale = (parsed.ragRationale || parsed.rationale || "").toString().trim();
  ...
  const locationDescription = parsed.locationDescription
    ...
    : undefined;
  const locationSource = parsed.locationSource ? parsed.locationSource.toString().trim() : undefined;
